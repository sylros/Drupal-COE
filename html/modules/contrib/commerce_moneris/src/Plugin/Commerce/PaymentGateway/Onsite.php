...
class Onsite extends OnsitePaymentGatewayBase implements OnsiteInterface {
  ...
  public function createPayment(PaymentInterface $payment, $capture = TRUE) {
    ...
    // kint($mpgResponse);
    $data = $mpgResponse->responseData;
    // kint($data);
    // kint($mpgResponse);
    // kint($mpgResponse->getEncUserId());
    // die();

    foreach ($data as $key => $value) {
      $fields['field_' . strtolower($key)] = $value;
    }

    $fields['type'] = 'transaction';
    $fields['title'] = 'Moneris Transaction';
    $fields['field_order_id'] = $payment->getOrderId();
    $fields['field_dynamic_descriptor'] = $this->dynamicDescriptor;
    $fields['field_crypt_type'] = 7;
    $fields['field_customer_id'] = $order->getCustomerId();
